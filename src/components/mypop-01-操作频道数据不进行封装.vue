<template>
    <div>
        ...
        <van-popup :value="value" @input="$emit('input', false)" position="bottom" :style="{ height: '90%' }" :overlay="true">
            ...
            <van-cell>
                <template slot="title">
                    <van-icon class="myclose" name="cross" />
                </template>
            </van-cell>
            <!-- 我的频道区域 -->
            <van-cell title="我的频道">
            ...
            <van-cell>
                <template slot="title">
                    <!-- 渲染频道数据 -->
                    <van-grid>
                        <van-grid-item class="topfather" v-for="(item, index) in channelList" :key="index">
                            <template slot="text">
                                <van-icon @click="delChannel(index)" v-if="iconshow" class="topson" name="clear" />
                                {{ item.name }}
                            </template>
                        </van-grid-item>
                    </van-grid>
...
export default {
    ...
    methods: {
        ...
        async addChannel (item) {
            ...
            // 判断用户是否登录
            if (user && user.token) {
                // 动态生成一个 channels 数组： [{ id: '', seq: 1 }]
                // 注意点：
                //      seq：默认从 1 开始
                //      推荐不允许操作，所以需要从数组中去掉，但是推荐的排序永远是 1，所以将来生成数据时，seq + 2
